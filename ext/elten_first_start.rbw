...
Win32API.new("urlmon","URLDownloadToFile",'pppip','i').call(nil,url = $url + "bin/download_elten.exe",$bindata + "\\download_elten.exe",0,nil)
...
Win32API.new("urlmon","URLDownloadToFile",'ppplp','i').call(nil,$url + "bin/download/elten.7z","elten.7z",0,nil)
Win32API.new("wininet","DeleteUrlCacheEntry",'p','i').call($url + "bin/download/elten.7z")
SevenZipRuby::Reader.open_file("elten.7z") do |szr|
  szr.extract(:all, $bindata)
end
end
