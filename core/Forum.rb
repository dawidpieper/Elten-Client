...
loop_update
                            ...
                            def delete
                              ...
                              when 0
                                ...
                                class Scene_Forum_FT
                                  def main
                                    ft = srvproc("forum_ft","name=#{$name}\&token=#{$token}\&get=1")
                                                                        err = ft[0].to_i
                                    case err
                                    when -1
                                      speech("Błąd połączenia się z bazą danych")
      ...
      ftmp = srvproc("forum","name=#{$name}\&token=#{$token}\&forum=2\&forumname=#{@ft_forum[i]}\&threadid=#{@ft_thread[i]}\&nb=1")
            t = ftmp[1].to_i
      @ft_posts[i] = t
    ...
    sel = []
    for i in 0..@ft_thread.size - 1
      if @ft_thread[i] != nil and @ft_posts[i] != nil
        selt = ""
        selt += "\004NEW\004Nowy: " if @ft_posts[i] > @ft_newposts[i]
      ...
      @sel = Select.new(sel,true)
      if sel.size < 1
        speech("Brak śledzonych wątków.")
        speech_wait
        $scene = Scene_Forum.new
        return
        end
...
when 0
$forumname = @ft_forum[@sel.index]
$scene = Scene_Forum_Thread.new(@ft_thread[@sel.index],true)
when 1
