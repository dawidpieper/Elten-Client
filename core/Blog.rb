...
class Scene_Blog
  ...
  speech("Blogi")
  speech_wait
  @sel = Select.new(["Mój blog","Lista blogów"])
  loop do
    loop_update
    @sel.update
    update
...
@sel = Select.new(sel,true,@categoryselindex)
loop do
  loop_update
  @sel.update
  update
...
$scene = Scene_Blog_My_Rename.new
  ...
  def menu
    ...
    @menu.disable_item(1) if @sel.index > $postid.size - 1
    loop do
      loop_update
      @menu.update
      if alt or escape
...
class Scene_Blog_My_Category_Delete
  ...
  def main
    speech("Czy jesteś pewien, że chcesz usunąć tę kategorię? Wszystkie wpisy w tej kategorii zostaną usunięte.")
    speech_wait
    if simplequestion == 0
      $scene = Scene_Blog_My.new
    else
...
loop do
  @form.update
  loop_update
        if (@form.index == 2 or $key[0x11] == true) and enter
...
speech_wait
bufid = buffer(text)
blogtemp = srvproc("blog_posts_mod","name=#{$name}\&token=#{$token}\&categoryid=#{@category}\&postid=#{@post}\&postname=#{postname}\&buffer=#{bufid}\&add=1")
err = blogtemp[0].to_i
if err < 0
  speech("Błąd.")
else
...
class Scene_Blog_My_Post_Delete
  ...
  def main
    speech("Czy jesteś pewien, że chcesz usunąć ten wpis?")
    speech_wait
    if simplequestion == 0
      $scene = Scene_Blog_My_Posts.new(@category,@categoryselindex,@postselindex)
    else
...
class Scene_Blog_My_Rename
  def main
    ...
    if blogname != "\004ESCAPE\004"
      bt = srvproc("blog_rename","blogname=#{blogname}")
      if bt[0].to_i == 0
...
@sel = Select.new(sel,true,$bloglistindex)
loop do
  loop_update
  @sel.update
  update
