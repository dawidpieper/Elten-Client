...
if Win32API.new("user32","GetAsyncKeyState",'i','i').call(0x20) != 0 and Input.trigger?(Input::C)
    ...
    def quit
      speech("Zamykanie programu")
      speech_wait
      sel = SelectLR.new(["Anuluj","Ukryj program w zasobniku systemowym","Wyjście"])
      loop do
        loop_update
        sel.update
        if enter
...
def setstatus(text)
  ...
  def buffer(data)
    return buffer_post(data) if data.size > 4000
        s=false
    while s==false
      s=true
      if data[data.size - 1..data.size - 1] == "\004" and data[data.size - 6..data.size - 6] == "\004"
        s=false
        for i in 1..6
        data.chop!
        end
    ...
    loop do
            t += 1
      if data[i..i+5] == "\004LINE\004"
                t -= 6
                end
                    bufdata[r] += data[i..i] if data[i..i] != nil
            if utf8(data[i..i + 1]) != data[i..i + 1] and data[i - 1..i] == data[i - 1..i] and utf8(data[i..i]) == "?"
              t -= 1
                    end
      if t >= 200
        r += 1
      ...
      buft = srvproc("buffer","name=#{$name}\&token=#{$token}\&ac=1\&id=#{bufid}\&data=#{bufdata[0]}")
            if buft[0].to_i < 0
        speech("Błąd")
        speech_wait
        $scene = Scene_Main.new
      ...
      for i in 1..bufdata.size - 1
              buft = srvproc("buffer","name=#{$name}\&token=#{$token}\&ac=2\&id=#{bufid}\&data=#{bufdata[i]}")
                          if buft[0].to_i < 0
        speech("Błąd")
        speech_wait
        $scene = Scene_Main.new
...
def avatar(user)
  avatartemp = srvproc("avatar","name=#{$name}\&token=#{$token}\&searchname=#{user}")
    case avatartemp[0].to_i
  when -4
    ...
    when -2
      ...
      tavatar = read("avatartemp")
              avatar = tavatar[3..tavatar.size - 1]
              cf = Win32API.new("kernel32","CreateFile",'piipiip','i')
